...
```text
A three-dimensional spectrogram waterfall: dozens of delicate glowing spectral ridgelines stacked one behind another and receding into deep perspective toward the right, each ridgeline an exquisitely fine jagged profile of peaks and valleys drawn as a hairline. The nearest ridges are the most defined; those further back fade progressively into atmospheric haze and darkness. Only a small number of individual peaks are picked out in hot coral; everything else is fine cyan and teal linework. The stack begins faint and sparse at the left.
```
